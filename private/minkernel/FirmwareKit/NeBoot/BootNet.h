// SPDX-License-Identifier: Apache-2.0
// Copyright 2024-2026, Amlal El Mahrouss (amlal@nekernel.org)
// Licensed under the Apache License, Version 2.0 (see LICENSE file)
// Official repository: https://github.com/ne-app-eu/krnl

#ifndef FIRMWAREKIT_BOOTNET_H
#define FIRMWAREKIT_BOOTNET_H

#include <NeKit/Config.h>

#define kBootNetINetMagic "ONET"
#define kBootNetINetMagicLength (4)
#define kBootNetVersion (0x001)
#define kBootNetNameLen (256U)

struct _BOOTNET_INTERNET_HEADER;

/// @brief Netboot Internet Header
/// Consists of 4 magic characters, and a set of fields describing the current patch that's being
/// sent (if m_preflight = 0)
/// @note Can be used to patch ROMs too (if ImpliesProgram = 1)
typedef struct _BOOTNET_INTERNET_HEADER {
  Ne::Kernel::Char NB1;  /// magic char 1 'O'
  Ne::Kernel::Char NB2;  /// magic char 2 'N'
  Ne::Kernel::Char NB3;  /// magic char 3 'E'
  Ne::Kernel::Char NB4;  /// magic char 4 'T'

  Ne::Kernel::UInt16 Version;

  Ne::Kernel::Char Name[kBootNetNameLen];  /// example: Modjo

  Ne::Kernel::Int32 Length;                   /// the patch length.
  Ne::Kernel::Char  Target[kBootNetNameLen];  /// the target file.

  Ne::Kernel::Boolean ImpliesProgram : 1;  /// does it imply reprogramming?

  Ne::Kernel::Boolean Preflight : 1;  /// is it a preflight packet.
  Ne::Kernel::Char    Data[1];        /// non preflight packet has a patch blob for a **PatchTarget**
} PACKED BOOTNET_INTERNET_HEADER;

using BOOTNET_INTERNET_HEADER_PTR = BOOTNET_INTERNET_HEADER*;

#endif
